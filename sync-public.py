...
if PUBLIC.resolve() == WORKING.resolve():
    print(f"ERROR: destination ({PUBLIC}) is the SAME directory as the source ({WORKING}).")
    print("This script must run from cc-toolkit-public/, NOT from ai-combined-tools/.")
    print("Refusing to sync a repo onto itself.")
    sys.exit(1)
...
SKIP_NAMES = {
    ...
    # Bug bounty credentials + cached program data (never sync)
    "bb_config.json",     # Encrypted bug bounty platform API tokens
    "bb_cache.json",      # Cached program scope data
    ...
    "test_cli_tools.sh",   # one-off client target diagnostics (feemcotech.solutions)
    "test_nuc_wap.sh",     # one-off client target diagnostics (feemcotech.solutions)
    ...
    # Dev/test scratch files (never sync)
    "_restart.py",
    "_restart.sh",
    "_test_render.py",
    "restart_dash.sh",
    "test_pd_wraps.py",
    "install-hooks.py",
    "exploit_orion.py",      # one-off target test

    # Scan/report artifacts generated by appsec scans (never sync)
    "cc_tool_index.json",
    "semgrep_ai-combined-tools.json",
    "semgrep_constants.json",
    "semgrep_S.json",
    "sbom_templates.cyclonedx.json",
    "sbom_web_dashboard.cyclonedx.json",
}

# Tool output directories with a timestamp suffix, e.g.
#   subfinder_output_20260902_185141_926027_0001/
#   httpx_output_20260905_000326_196229_0006/
# These are runtime recon outputs containing target data — NEVER sync.
SKIP_DIR_PATTERNS = [
    re.compile(r"^(httpx|subfinder|nuclei|katana|naabu|ffuf|gobuster)_output_\d+_\d+"),
    re.compile(r"^output_\d{8}_\d{6}"),
    re.compile(r".*_output_\d{8}_\d{6}.*"),
]
...
FILE_BLOCKLIST = frozenset({
    ...
    "bb_config.json",     # Bug bounty platform API credentials (encrypted tokens)
...
ENV_PATTERNS = [
    ...
    (r"feemcotech", "Hardcoded client domain"),
    ...
    (r"/share/", "Hardcoded /share mount path"),
    ...
    for f in path.rglob("*"):
        ...
        for part in rel.parts:
            if part in SKIP_NAMES:
                ...
                break
            if any(p.search(part) for p in SKIP_DIR_PATTERNS):
                in_skipped = True
                violations.append(f"  INSIDE-BLOCKED-DIR: {rel} (under output dir '{part}/')")
                break
        if in_skipped:
            continue
...
def main():
    ...
    # -- Safety scan: verify destination has no leaked data --
    ...
    for src_path in sorted(WORKING.rglob("*")):
        ...
        # Skip data directories (exact names + timestamped output dirs)
        if SKIP_NAMES & parts:
            skipped += 1
            continue
        if any(p.search(part) for part in parts for p in SKIP_DIR_PATTERNS):
            skipped += 1
            continue
